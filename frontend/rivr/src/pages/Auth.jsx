...
function Auth() {
    ...
    const handleDataVerification = async () => {
        try {
            ...
            // Only handle the code for development convenience
            if (response.data && response.data.code) {
                console.log("Verification code from server:", response.data.code);
                // Pre-fill the verification code for ease of testing in development
                const codeArray = response.data.code.split('').slice(0, 6);
                setVerificationCode(codeArray.concat(Array(6 - codeArray.length).fill('')));
            }
            
            setIsConfirmingEmail(true);
        } catch (err) {
            alert(err.response?.data?.email || "Something went wrong");
        }
    };
